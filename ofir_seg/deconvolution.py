...
def construct(image_lmdb, contour_lmdb, batch_size=1):
    ...
    net.conv2 = L.Convolution(net.pool1, kernel_size=3, stride=1, num_output=100,
                              pad=0, group=1, weight_filler=weight_filler,
                              bias_filler=bias_filler, param=param)
    ...
    net.drop = L.Dropout(net.pool2, dropout_ratio=0.1, in_place=True)

    net.conv3 = L.Deconvolution(net.drop) #, kernel_size=5, stride=2, num_output=50,
    #                          pad=0, weight_filler=weight_filler,
    #                          bias_filler=bias_filler, param=param)
    net.norm3 = L.BatchNorm(net.conv3)
    net.relu3 = L.ReLU(net.norm3, in_place=True)

    net.conv4 = L.Deconvolution(net.relu3) #, kernel_size=6, stride=2, num_output=1,
    #                          pad=1, weight_filler=weight_filler,
    #                          bias_filler=bias_filler, param=param)
    
    net.out = L.Sigmoid(net.conv4, in_place=True)

    net.loss = L.EuclideanLoss(net.out, net.label)
...
def main():
    ...
    with open('decon_train.prototxt', 'w') as f:
        net_proto = construct(
            'train_image_lmdb/',
            'train_contour_lmdb/',
            batch_size=16)
    ...
    with open('decon_test.prototxt', 'w') as f:
        net_proto = construct(
            'test_image_lmdb/',
            'test_contour_lmdb/')
        f.write(header + str(net_proto))
